...
//
    ...
    LinearList(LinearList<ListType> &l) {
        ...
        for (uint32_t i=0;i<l.count;i++) {
            ptr = new ListNode<ListType>;   // make a new node
            ptr->datum = tmp->datum;        // copy one value from existing list
            *prev = ptr;                    // attach new node to end of new list
            prev = &((*prev)->next);        // move pointer (to pointer) to end of new list
            tmp = tmp->next;                // move to next node in existing list
        }
